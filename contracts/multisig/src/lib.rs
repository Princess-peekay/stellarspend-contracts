...
/// Updates the threshold above which high-value rules apply. Admin-only.
...
/// Panics with `MultisigNotConfigured` if the multisig is not ready to operate.
pub fn ensure_multisig_configured(env: &Env) {
    let signers = get_signers(env);
    let threshold = get_threshold(env);
    if signers.is_empty() || threshold == 0 || threshold > signers.len() {
        panic_with_error!(env, Error::MultisigNotConfigured);
    }
}

/// Panics if `signer` is not in the configured signer set.
...
/// Increments and returns the next transaction ID. Panics on overflow.
...
/// Returns `true` if `signer` has already approved `tx_id`.
...
/// Records an approval from `signer` for `tx_id`. Returns the new approval count.
///
/// Panics with `DuplicateApproval` if the signer has already approved.
/// Panics with `Overflow` if the count would wrap.
...
/// Validates that `signers` and `threshold` form a legal configuration.
fn validate_signer_config(env: &Env, signers: &Vec<Address>, threshold: u32) {
    ...
    for i in 0..signer_count {
        let signer_i = signers
            .get(i)
            .unwrap_or_else(|| panic_with_error!(env, Error::InvalidThreshold));
